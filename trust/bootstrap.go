...
// a directory of PEM/DER files.
func LoadCertsFromPath(path string) ([]*x509.Certificate, error) {
	certs, _, err := loadBootstrap(path)
	return certs, err
}

// loadBootstrap loads the bootstrap certificates and, when the source is a
// signer manifest, its declared oj_reference (empty for a plain PEM/DER file or
// directory). Both callers share the one file read.
func loadBootstrap(path string) ([]*x509.Certificate, string, error) {
...
// (and, for a manifest, its declared oj_reference). A LOTL signer manifest — a
// structured "signers:" list with each certificate embedded as a PEM block — is
// parsed as such; any other file is read as raw PEM/DER certificate material.
// The manifest is recognised by a .yaml/.yml extension or a top-level
// "signers:" key, so the pinned path can point at either shape (a manifest, or
// a plain concatenated PEM).
func parseBootstrapFile(path string, raw []byte) ([]*x509.Certificate, string, error) {
...
// block, plus the OJ notice they were published in. The manifest also carries
// human metadata (issuer, validity, fingerprints) that is informational and
// cross-checked out of band before a change is trusted.
type signerManifest struct {
	OJReference string           `yaml:"oj_reference"`
...
// certificates and its declared oj_reference. Fail-closed: malformed YAML, an
// entry missing its certificate, or an entry whose PEM does not hold exactly
// one certificate is an error — a partial or ambiguous bootstrap set is never
// returned.
func loadSignerManifest(raw []byte) ([]*x509.Certificate, string, error) {
	...
	return out, m.OJReference, nil
}

// SeedBootstrap builds the version-1 bootstrap state from the operator-pinned
// signer set at path (LOTL_BOOTSTRAP_CERTS_PATH). When the source is a signer
// manifest, its declared oj_reference is recorded on the bootstrap; a plain
// PEM/DER path records no reference.
func SeedBootstrap(path string, now time.Time) (*Bootstrap, error) {
	certs, ojRef, err := loadBootstrap(path)
	if err != nil {
		return nil, fmt.Errorf("trust: load bootstrap certs: %w", err)
	}
	b := &Bootstrap{Version: 1, OJReference: ojRef, ActivatedAt: now, Seeded: true}
	for _, c := range certs {
