...
Tests run against recorded fixtures under [testdata/](testdata) and do not touch the
network. A change to list parsing or signature verification should come with a fixture
that fails without it; see [SPECREFS.md](SPECREFS.md) for the exact standard editions the
fixtures and comments are checked against, and cite clauses in the same bracket form.
